...
# Fetch logs for the recent list
recent_records = get_conversations(limit=20)

for record in recent_records:
    st.write(f"**{record.prompt[:80]}...**")
    st.write(f"{record.answer[:200]}...")
    # Fixed record.record_time to record.response_time
    st.write(f"Response Time: {record.response_time:.2f}s | Cost: ${record.cost:.4f}")
    st.divider()
...
coll.metric("Thumbs up", int(thumbs_up or 0))
